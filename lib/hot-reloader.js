...
function HotReloaderFactory ({getEmitter, globals = {}} = {}) {
  return class HotReloader extends Emitter {
    constructor (backendUrl, transform = identity, fileChangeEmitter) {
      if (!backendUrl && globals.document) {
        backendUrl = '//' + globals.document.location.host
      }
      if (!fileChangeEmitter) {
        fileChangeEmitter = getEmitter(backendUrl)
      }
      super()
      ...
      this.fileChangeEmitter = fileChangeEmitter
      fileChangeEmitter.on('connect', () => {
        d('hot reload connected to watcher on ', backendUrl)
        fileChangeEmitter.emit('identification', typeof navigator !== 'undefined' ? navigator.userAgent : 'node')
        fileChangeEmitter.emit('package.json', function (pjson) {
          // self.pjson = pjson // maybe needed in the future?
          self.jspmConfigFile = pjson.jspm.configFile || 'config.js'
        })
      })
      fileChangeEmitter.on('reload', () => {
        d('whole page reload requested')
        globals.document.location.reload(true)
      })
      fileChangeEmitter.on('change', (...args) => this.onFileChanged(...args))
      if (typeof globals.window !== 'undefined') {
        globals.window.onerror = (err) => {
          fileChangeEmitter.emit('error', err)  // emitting errors for jspm-dev-buddy
        }
      }
      fileChangeEmitter.on('disconnect', () => {
        d('hot reload disconnected from ', backendUrl)
      })
      if (System.loads) this.pushImporters(System.loads)
    }
    onFileChanged (ev) {
...
const SimpleHotReloader = HotReloaderFactory()
