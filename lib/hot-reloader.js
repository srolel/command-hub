...
class HotReloader extends Emitter {
  constructor (emitter, transform = identity) {
    if (!emitter) {
        throw new Error('emitter must be provided');
    }
    super()
    ...
    this.emitter.on('connect', () => {
      d('hot reload connected to watcher')
      this.emitter.emit('identification', navigator.userAgent)
      this.emitter.emit('package.json', function (pjson) {
    ...
    this.emitter.on('change', this.onFileChanged.bind(this))
    window.onerror = (err) => {
      this.emitter.emit('error', err)  // emitting errors for jspm-dev-buddy
    }
    this.emitter.on('disconnect', () => {
      d('hot reload disconnected')
    })
  ...
  deleteModule (moduleToDelete, from) {
    ...
    if (!this.modulesJustDeleted[name]) {
      ...
      if (typeof exportedValue.__reload === 'function') {
        exportedValue.__reload(moduleToDelete) // calling module reload hook
      }
      System.delete(name)
